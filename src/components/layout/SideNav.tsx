...
export default function SideNav({ activeId, onSelect, className = '' }: SideNavProps) {
    return (
        <main className={`glass rounded-md flex flex-col items-center p-md h-full gap-lg ${className}`}>
            <header>
            ...
            <nav className="flex flex-col w-full">
                {navLinks.map(({ label, sectionId, description }) => {
                    ...
                    return (
                        <div
                            key={sectionId}
                            className={`flex flex-col rounded-md transition-all duration-300 ease-in-out ${isActive ? 'glass p-xs' : 'p-xs'}`}
                        >
                            <Button
                                label={label}
