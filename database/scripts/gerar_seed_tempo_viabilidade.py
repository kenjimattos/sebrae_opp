#!/usr/bin/env python3
"""
Gera o seed (mongosh) do indicador **Tempo de viabilidade da empresa** (`tempo-viabilidade`)
...
(sem autenticação) em https://estatistica.redesim.gov.br/tempos-abertura-redesim/
  ...
  - GET /percentual-viabilidade/{UF}?ano=&mes=      -> agregado UF (cross-check)
...
INDICADOR: `tempo-viabilidade` = tempo da **etapa de viabilidade** (pesquisa prévia de
nome e endereço), coluna QTDE HH 11 do XLSX, em horas úteis.

> O irmão `tempo-abertura` (tempo total = viabilidade + validação cadastral + registro)
> tem seu **próprio** gerador `gerar_seed_tempo_abertura.py` — mesma fonte, scripts
> independentes (um indicador não depende do outro), seguindo o padrão "uma fonte → um
> script → um seed" do projeto.
...
Cobertura: os 223 municípios da PB. Municípios sem nenhuma solicitação na janela (n=0)
...
Saídas (idempotentes, próprias para o NoSQLBooster):
  - database/seed/indicador-tempo-viabilidade.mongodb.js   (catálogo + valores)
  - database/data/tempo_viabilidade_pb.json                (snapshot versionado dos agregados)

Uso:
  python3 database/scripts/gerar_seed_tempo_viabilidade.py            # online (baixa 12 XLSX)
  python3 database/scripts/gerar_seed_tempo_viabilidade.py --offline  # usa o snapshot salvo
"""
...
#   11 = QTDE. HH VIABILIDADE TOTAL
COLS_TEMPO = (11,)
CROSSCHECK_ENDPOINT = "percentual-viabilidade"
SNAPSHOT = DATA_DIR / "tempo_viabilidade_pb.json"
SEED_FILE = SEED_DIR / "indicador-tempo-viabilidade.mongodb.js"
...
def build_values(snapshot: dict) -> list[dict]:
    ...
    for code in codes:
        a = aggr.get(code)
        if not a:  # sem nenhuma solicitação na janela
            valores.append({
...
HEADER = """// ARQUIVO GERADO por database/scripts/gerar_seed_tempo_viabilidade.py — NÃO editar à mão.
