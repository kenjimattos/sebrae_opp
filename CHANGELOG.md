...
- **O semáforo classificava o texto, não o número — 11 municípios acendiam a cor errada.** O ETL calcula um float por município e grava dois campos derivados dele: `numericValue`, o número na precisão da fonte, e `rawValue`, a string de exibição em padrão BR, arredondada. A API classificava pelo `rawValue`, reconstituindo um número a partir da string com um heurístico de formato brasileiro — e o `numericValue`, presente em 100% dos documentos, estava declarado em `IndicatorValueDoc` sem uma única leitura no código. O problema não é o heurístico: é que os cortes oficiais discriminam justamente na casa decimal que o arredondamento da exibição come. IGM 5,008 vira `"5,01"` e o corte do CFA é 5,01; ISDEL 0,470942 vira `"0,471"` e o corte é 0,471. Cinco municípios no `igm-cfa` (2502102, 2507408, 2511905, 2512309, 2515302), cinco no `isdel-governanca` (2503902, 2516102, 2508307, 2510907, 2513356) e um no `tempo-viabilidade` (2512762) apareciam melhores do que são — todos errando para o lado otimista, porque arredondar aproxima do corte por cima. `computeStatus` passa a receber o documento e ler `numericValue`; o caminho `enum` segue lendo o texto, que lá é o dado certo (a faixa é um mapa de rótulos).
