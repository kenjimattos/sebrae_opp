...
- **`GET /api/emendas` ainda não tem rota no backend.** O modo Emendas do "Mapeamento de recursos" chama `/api/emendas`, mas a rota Fastify em `server/` não existe — o painel fica sem dado até ela ser implementada lendo a coleção `emendas` (seeds em `database/seed/emendas-{federais,estaduais}.mongodb.js`) e devolvendo o shape de `src/types/emendas.ts`.
