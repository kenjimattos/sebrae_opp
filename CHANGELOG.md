...
- **A plataforma tem nome: PIPPA.** O hero do Login (Figma 1630:40) passa a abrir com o logotipo no lugar do antigo `<h1>` "Observatório de Políticas Públicas" e do parágrafo de macro-objetivo; acima dele, entre dois filetes, a sigla aberta — "Plataforma de Inteligência de Políticas Públicas Aplicada". O parágrafo não sumiu do produto: `hero.description` continua servindo a SideNav, que é onde ele ainda tem trabalho a fazer. O `<title>` da aba acompanha. Duas coisas do desenho **não** foram seguidas, de propósito: o botão "Entrar" continua na tela (o Figma não o desenha, e sem ele não há como entrar) e os cards seguem não-clicáveis (no Login não há município escolhido nem sessão — não existe destino para onde levar).

- **O logotipo é contorno vetorial, não webfont.** A Norfolk Narrow Regular, a fonte do desenho, é licenciada só para uso pessoal: servi-la no bundle seria redistribuição. Então o traço das cinco letras foi extraído do `.otf` com fontTools (avanço de cada glifo + os 0,04em de tracking do Figma, eixo Y invertido) e vive como `path` em `src/components/brand/PippaWordmark.tsx`. São 900 bytes contra ~20 KB de fonte, não há FOUT no maior elemento da tela, e `fill="currentColor"` resolve os dois temas sem um segundo arquivo. O `.otf` fica no `.gitignore` — está na máquina de quem desenha, não no repo.

- **O filete do eyebrow nunca tinha aparecido na tela.** O `<hr className="w-[4rem]">` do hero antigo era invisível: o preflight do Tailwind zera a borda de todo elemento, `<hr>` incluído, e o que sobra é uma caixa de altura zero. Os filetes novos usam `.divider`, que pinta com `--semantic-divider` em vez de depender de borda.

- **Os cards dos 4 pilares estavam 90px mais estreitos que o desenho.** A grade tinha `max-w-[70dvw]` e `px-3xl` no mesmo elemento — com `box-sizing: border-box`, os 96px de padding de cada lado entram no `max-width`, e sobravam 376px por card contra os 466px do Figma. É a armadilha que o `CLAUDE.md` já registra. Com o padding fora e a largura vinda da conta do desenho (2×466 + 64 de gap), os títulos voltam a caber em uma linha só. Título e descrição também vieram para o tamanho do Figma (20px e 12px, contra 16px nos dois).

### Infraestrutura

- **`server/src/` passa a se organizar por domínio.** Os três arquivos que cresciam juntos estavam colados por camada, não por assunto: `services.ts` (266 linhas) guardava os builders de indicadores, mapa e emendas sem nada em comum além do import, `repo.ts` misturava catálogo com municípios e emendas, e `types.ts` (233 linhas) juntava duas famílias que mudam por motivos diferentes — o shape que o ETL grava e o contrato que o frontend consome. Agora são `indicadores/`, `municipios/`, `mapa/` e `emendas/`, mais `infra/` e `types/` (`docs.ts` + `api.ts`). A escolha foi por domínio e não por MVC (`controllers/`, `models/`, `services/`) porque o que cresce aqui é pilar novo — emendas nasceu inteira, com repo, service, tipos e a regra própria de zero-vs-`null` —, e num MVC clássico esse trabalho se espalha por quatro pastas em vez de caber numa. Também não havia "model" a povoar um `models/`: os docs do Mongo são dados puros, sem comportamento. As regras compartilhadas pelas duas leituras do mesmo dado (a régua do status, a escolha do ano e a supressão por amostra baixa) ficaram em `indicadores/`, visíveis como compartilhadas: duplicá-las faria o mapa discordar da ficha que ele abre, e a discordância apareceria na tela, não no teste. **Nenhuma lógica mudou** — conferido por diff linha a linha contra os arquivos antigos: as únicas diferenças fora de `import` são duas palavras `export`, nos dois helpers que passaram a cruzar a fronteira de módulo.
...
- **O `server/` desta branch passa a ser o mesmo da `main`.** Ele estava uma versão atrás de propósito — sem `GET /api/emendas` e sem `POST /api/ai` — sob a lógica de que aqui a API não atende ninguém, já que `/api/*` vem do snapshot estático. O port da 1.4.1 mostrou o custo dessa escolha: `tests/server/emendas.test.ts` compila contra `server/src/` e a suíte é a mesma nas duas branches, então o atraso derrubou `tsc -b`, `npm run build` e 19 dos 156 testes. Manter o código atrasado não protegia nada e quebrava o build. São 10 arquivos, quase tudo código que faltava; o que esta branch tinha de próprio era o aviso no `server/README.md`, `rootDir: "src"` e os scripts sem o shim de entrypoint. A partir daqui as duas branches diferem **só** na camada que serve dados, que é a razão de esta existir.
...
- **Avaliação de Storybook (`docs/avaliacao-storybook.md`).** Veredito: viável e o projeto está bem posicionado — os 17 primitivos de `ui/` não consomem contexto, as variantes já são enumeráveis e exportadas, e `public/api-snapshot/` dá fixtures reais sem rede —, mas **não urgente**, porque o saneamento desta versão resolveu por outros meios a dor que motivaria a adoção (paridade de tema auditável por diff, contraste medido, regra de lint). O documento registra o que o setup exigiria sem falha (importar o `index.css` no preview, senão as classes compostas somem em silêncio; filtrar o plugin `/api/ai` e o proxy do `vite.config.ts`; `staticDirs` para os snapshots), o risco do Vite 8 ainda recente sob o Rolldown, o custo por etapa e a recomendação de adotar quando houver trabalho de UI em lote, não antes.

- **Os dois READMEs desatualizados foram reescritos.** O da raiz não era tocado desde 08/07 (antes do 1.2.0) e descrevia um projeto que não existe mais: mandava subir a API Node e o proxy do Vite — nesta branch `/api/*` vem do snapshot estático —, dizia que "o frontend não precisa de variáveis de ambiente" quando precisa da `OPENROUTER_API_KEY`, e listava a rota `/oportunidades`, as páginas `Opportunities`/`Community`, o modo Editais e o `/trilhas` como carrossel — tudo removido ou substituído. Não mencionava IA, emendas, chat, a pasta `api/` nem a própria branch `preview/snapshot`. Agora abre declarando em que branch o leitor está e para onde vai o backend em cada uma, tem seções de **Backend nesta branch** e **Integração de IA**, e a árvore de `src/` volta a bater com o disco.
- **`database/README.md`: emendas, os 33 indicadores e o snapshot.** O arquivo cobria 19 dos 38 seeds e não citava emendas em lugar nenhum, apesar dos dois seeds, dos dois geradores e da coleção `emendas` existirem — quem montasse o banco pela lista terminava com um banco parcial, sem erro nenhum. Também afirmava "4 coleções" (são 5) e conferia contagens de uma carga antiga (12 indicadores / 8.251 valores; hoje 33 / 12.934). Passa a listar os 38 seeds agrupados por agenda e base econômica, marcando os 6 que têm `threshold`, e ganha o schema da coleção `emendas` com as duas armadilhas que só o validador registrava (o eixo de `porAno` muda por esfera; zero federal é zero, ausência estadual é `null`), a operação dos dois ETLs de emendas com o limite do método estadual, e a seção do snapshot estático — incluindo o fato de que os 223 JSONs de municípios não têm gerador versionado.
...
- **`CLAUDE.md` enxugado e alinhado à realidade desta branch (259 → 177 linhas).** Mesmo enxugamento feito na `main` — saíram o dump das CSS vars que vive no `index.css`, a enumeração de props do `ParaibaOutlineMap`, o algoritmo do `ModeRisks`, a interface copiada da fonte e a tabela de `src/data/` que era listagem de diretório. Além disso, a seção de backend passou a descrever o que de fato acontece aqui: a Vercel não alcança o Mongo do Sebrae, então `/api/*` vem do snapshot estático em `public/api-snapshot/` via `rewrites` do `vercel.json` e `bypass` do `vite.config.ts` — que precisam ser espelho um do outro, senão a rota funciona no preview e quebra em dev. O arquivo antes descrevia a API Fastify como se ela servisse esta branch, e não mencionava o snapshot em nenhum lugar.
